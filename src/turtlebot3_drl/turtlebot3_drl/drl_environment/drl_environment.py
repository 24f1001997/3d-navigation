...
class DRLEnvironment(Node):
    ...
    *******************************************************************************"""
    ...
    def initalize_episode(self, response):
        ...
        corners = [
            (ARENA_LENGTH/2, ARENA_WIDTH/2),
            (ARENA_LENGTH/2, -ARENA_WIDTH/2),
            (-ARENA_LENGTH/2, ARENA_WIDTH/2),
            (-ARENA_LENGTH/2, -ARENA_WIDTH/2)
        ]
        d_max = max([math.hypot(self.goal_x - cx, self.goal_y - cy) for cx, cy in corners])
        
        rw.reward_initalize(self.initial_distance_to_goal, d_max)
        return response
